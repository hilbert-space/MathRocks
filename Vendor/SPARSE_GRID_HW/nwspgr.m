...
%  Create cell arrays N1D and W1D containing the node and weight information.
%  The array R1D stores the length or order of each rule.
%
...
%
    for j = 1 : size(is,1)
      midx = is(j,:);
      [ newn, neww ] = tensor_product ( n1D(midx), w1D(midx) );
