function assessDiscontinuity
  ...
  [ sgOutput, ~, sg ] = assess(target, sgOptions, ...
    'sampleCount', sampleCount, ...
    'exactExpectation', sqrt(15 / 35) / 4, ...
    'exactVariance', 45 / 112);

  y = sg.evaluate(sgOutput, transpose((z + 1) / 2));
  plotSlice(t, z, y, Y(k, :));

  Statistic.observe(sgOutput.data, 'draw', true, 'method', 'histogram');
end
